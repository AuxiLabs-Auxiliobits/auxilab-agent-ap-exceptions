...
| Parameter | Default | Description |
|-----------|---------|-------------|
| `default_tax_rate` | `0.00` (0%) | Default tax rate when currency not found in rates table |
| `validate_tax_id_checksum` | `true` | Whether to validate tax ID checksum |
| `tax_id_format` | `"EIN"` | Tax ID format for validation. Options: `"EIN"`, `"ABN"`, `"NONE"` |
...
**US EIN (9 digits):**
1. Normalize: remove all non-numeric characters (like hyphens)
2. Verify length is exactly 9 digits
3. Valid if contains only numbers

**Supported Tax ID Formats:**

| Format | Validation |
|--------|------------|
| `EIN` | Format validation (algorithm above) |
| `ABN` | Full checksum validation (modulo 89) |
| `NONE` | Validation disabled — always passes |

> **Note:** VAT format is configured but not yet implemented. It currently returns valid by default.
