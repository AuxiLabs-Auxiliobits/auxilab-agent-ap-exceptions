...
def validate_ein_checksum(ein: str) -> tuple[bool, str]:
    """Validate US Employer Identification Number format"""
    ein_clean = normalize_tax_id(ein)

    if len(ein_clean) != _EIN_EXPECTED_LENGTH:
        return (
            False,
            f"Invalid length: {len(ein_clean)} (must be {_EIN_EXPECTED_LENGTH})",
        )
...
def validate_tax_id(tax_id: str, format_type: str = "EIN") -> tuple[bool, str]:
    ...
    if format_type == "EIN":
        return validate_ein_checksum(tax_id)

    # Add other formats as needed (VAT, etc.)
    ...
    def run(
        ...
        if invoice_sources:
            ...
            try:
                ...
                print("=== RAW GEMINI EXTRACTION OUTPUT ===")
                print(f"invoice_number: {invoice_data.get('invoice_number')}")
                print(f"po_number: {invoice_data.get('po_number')}")
                print(f"vendor_name: {invoice_data.get('vendor_name')}")
                print(f"invoice_amount: {invoice_data.get('invoice_total_inc_tax')}")
                print(f"invoice_date: {invoice_data.get('invoice_date')}")
...
- Extract invoice_number from text patterns like: "Invoice No:", "Invoice #:", "INV-", "Invoice Number:". Do NOT use the filename as the invoice number under any circumstance.
- Extract po_number from text patterns like: "PO Number:", "Purchase Order:", "PO#:", "PO-". If the value is "N/A" or blank on the document, then set as empty string. Otherwise extract the exact alphanumeric value present.
