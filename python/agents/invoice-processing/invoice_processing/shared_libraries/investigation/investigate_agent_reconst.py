...
class ReconstructedRulesExtractor:
    ...
    def get_ein_validation_rules(self) -> str:
...
"""
    ...
    def validate_ein_matching(
        self,
        extracted_ein: str,
        reference_ein: str,
        ein_validation_metadata: dict,
        currency: str,
        agent_decision: str,
    ) -> dict:
        """
        Validate EIN matching per Section 3.8 and Step 22.

        Key validations:
        - Two-path validation (ExtractorAgent format check + Phase 3 validation)
        - EIN format algorithm for USD (9 digits)
        - Re-extraction attempt if format fails
        - Hamming distance analysis for OCR error detection
        """
        ein_rules = self.rules_extractor.get_ein_validation_rules()

        prompt = f"""Validate EIN matching per reconstructed_rules_book.md Section 3.8 and Step 22.

**IMPORTANT: SCOPE OF THIS VALIDATION**
This validates ONLY Step 22 (EIN matching). Step 22.1 (Vendor Name) is validated SEPARATELY.
The agent's final decision may be SET_ASIDE due to Step 22.1 (vendor name mismatch) even if EIN passed!
You are ONLY checking if the EIN validation logic was correct, NOT the final Phase 3 decision.

**CASE DATA:**
- Extracted EIN: {extracted_ein}
- Reference EIN: {reference_ein}
- Currency: {currency}
- Agent Final Decision: {agent_decision} (may be influenced by Step 22.1, not just EIN)

**EIN VALIDATION METADATA:**
```json
{json.dumps(ein_validation_metadata, indent=2)[:1500]}
```
...
- For USD: Validate EIN format (must be exactly 9 digits)
- If format fails: Attempt re-extraction with Gemini Pro
- Store result in ein_validation_metadata

**Path 2: Phase 3 Validator (Agent 5)**
- Use pre-validated EIN from ExtractorAgent
...
**CRITICAL: WHAT "valid" MEANS FOR EIN**
- "valid" = TRUE means the EIN validation logic (Step 22) was performed correctly
- "valid" = FALSE means the EIN validation logic had an error
- "expected_decision" is what Step 22 should output (ACCEPT if EIN matches, REJECT/SET_ASIDE if not)
- If EIN matched exactly AND agent's final decision is SET_ASIDE → valid=TRUE (because Step 22 passed,
  but Phase 3 failed on Step 22.1)

**EXAMPLE: EIN Passes but Agent SET_ASIDE (due to vendor name)**
- Extracted EIN: 123456789
- Reference EIN: 123456789 (exact match!)
